...
def register_update_annotations_on_graph(graph_id, checkboxes_id, page_selector_id):
    @callback(
    ...
    def _update_annotations_on_graph(
        annotations_to_show, fig_dict, page_selection, annotations, chunk_limits
    ):
        """Update annotations visibility based on the checklist selection."""

        if (
            not annotations_to_show
            or len(annotations_to_show) == 0
            or not fig_dict["data"]
        ):
            fig_patch = Patch()
        ...
        if dash.callback_context.triggered[0]["prop_id"].split(".")[
            0
        ] == graph_id and fig_dict["layout"].get("shapes", []):
            return dash.no_update

        return gu.update_annotations_on_graph(
            fig_dict, annotations_to_show, page_selection, annotations, chunk_limits
        )


def register_update_annotation_graph(
    update_button_id, page_selector_id, checkboxes_id, annotation_graph_id
):
    @callback(
    ...
    def update_annotation_graph(
        ...
        if (
            not n_clicks
            or not annotations_data
            or not isinstance(annotations_data, list)
            or not annotations_to_show
            or not isinstance(annotations_to_show, list)
        ):
            return dash.no_update
        ...
        for _, row in annotations_df.iterrows():
            if row["description"] in annotations_to_show:
                if row["duration"] == 0:
                    tick_vals.append(
                        row.name
                    )  # Use the onset time as the tick position
...
def register_move_with_keyboard(keyboard_id, graph_id, page_selector_id):
    @callback(
        Output(graph_id, "figure", allow_duplicate=True),
...
def register_move_to_next_annotation(
    prev_spike_id, next_spike_id, graph_id, dropdown_id, checkboxes_id, page_selector_id
):
    @callback(
        Output(graph_id, "figure", allow_duplicate=True),
    ...
    def _move_to_next_annotation(
        ...
        if not annotations_data or not annotations_to_show:
            return (
                dash.no_update,
                dash.no_update,
            )  # No annotations available, return the same graph
        ...
        spike_x_positions = [
            ...
            if ann["description"] in annotations_to_show
        ]
