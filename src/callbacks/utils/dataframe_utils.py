def get_raw_df_filtered_on_time_dask(time_range, raw_ddf):
    raw_ddf = raw_ddf.reset_index()
    raw_ddf["time_seconds"] = (
        raw_ddf["time"] - raw_ddf["time"].iloc[0]
    ).dt.total_seconds()

    filtered_ddf = raw_ddf[
        (raw_ddf["time_seconds"] >= time_range[0])
        & (raw_ddf["time_seconds"] <= time_range[1])
    ]
