...
def load_and_process_meg_data(
    ...
    try:
        if ".ds" in file_path:
            raw = (
                mne.io.read_raw_ctf(file_path, preload=False)
                .pick(picks=["meg"], exclude="bads")
                .load_data()
            )
            USE_REFERENCE_CHANNELS = True
        elif ".fif" in file_path:
            raw = (
                mne.io.read_raw_fif(file_path, preload=False)
                .pick(picks=["meg"], exclude="bads")
                .load_data()
            )
        ...
        for pattern, channels in config.get("special_case_handling", {}).items():
            if pattern in file_path:
                ...
                if channels_to_drop:
                    ...
                    logger.info(
                        f"Dropped {len(channels_to_drop)} special case channels for pattern '{pattern}'"
                    )

        if good_channels is None or not USE_REFERENCE_CHANNELS:
            good_channels = list(
                raw.ch_names
            )  # Use all available channels if no reference provided
            logger.info(
                f"No good_channels provided, using all {len(good_channels)} available channels"
            )
        ...
        if config.get("notch_freq", 50.0) > 0:
            freqs = np.arange(
                config["notch_freq"], config["sampling_rate"] / 2, config["notch_freq"]
            ).tolist()
        ...
        raw_data = np.array(
            raw.get_data()
        )  # Shape: (n_selected_channels, n_timepoints)
        ...
        if config.get("median_filter_temporal_window_ms", 0) > 0:
            raw_data = apply_median_filter(
                raw_data,
                config["sampling_rate"],
                config["median_filter_temporal_window_ms"],
            )
        ...
        for ch_idx, ch_name in enumerate(channel_info["selected_channels"]):
            if ch_name in good_channels_index:
                ...
                logger.warning(
                    f"Channel {ch_name} not in good_channels reference - skipping"
                )

        n_valid = channel_mask.sum().item()
        logger.debug(
            f"Channel masking (file: {os.path.basename(file_path)}): {n_valid}/{len(good_channels)} valid channels"
        )

        # Store channel mask for batch collation
        channel_info["channel_mask"] = channel_mask
        channel_info["USE_REFERENCE_CHANNELS"] = USE_REFERENCE_CHANNELS and (
            good_channels is not None
        )
...
def select_channels(
    raw: BaseRaw, good_channels: List[str]
) -> Tuple[BaseRaw, Dict[str, Any]]:
    ...
    if len(selected_channels) == 0:
        raise ValueError(
            f"No channels from good_channels found in raw data! "
            f"Raw has: {list(raw.ch_names)[:10]}..., "
            f"Expected: {good_channels[:10]}..."
        )

    logger.debug(
        f"Selected {len(selected_channels)}/{len(good_channels)} channels from reference list"
    )
...
def normalize_data(
    data: np.ndarray, norm_config: Dict, eps: Optional[float] = None
) -> np.ndarray:
    ...
    if method == "percentile":
        ...
        raise ValueError(
            f"Unknown normalization method: {method}. Supported methods: percentile, zscore, minmax, robust_normalize, robust_zscore"
        )


def apply_median_filter(
    data: np.ndarray, sfreq: float, temporal_window_ms: float
) -> np.ndarray:
...
def find_gfp_peak_in_window(
    meg_data: np.ndarray, window_start: int, window_end: int, sampling_rate: float
) -> Tuple[int, float]:
...
class PredictDataset(torch.utils.data.Dataset):
    ...
    def _load_recording(self):
        ...
        try:
            ...
            self.n_chunks = (
                total_windows + num_context_windows - 1
            ) // num_context_windows

            self.logger.info(
                f"Loaded recording: {self.meg_data.shape[1]} samples, "
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        end_window_idx = min(
            start_window_idx + num_context_windows, len(self.all_windows)
        )
        ...
        metadata = {
            ...
            "chunk_offset_sample": chunk_onset_sample
            + len(windows) * window_step
            + (window_duration_samples - window_step),
            "chunk_duration_samples": len(windows) * window_step
            + (window_duration_samples - window_step),
            "chunk_idx": idx,
            ...
            "patient_id": (
                self.file_path.split("/")[-2] if "/" in self.file_path else "unknown"
            ),
            "channel_mask": (
                self.channel_info.get("channel_mask", None)
                if self.channel_info
                else None
            ),
            "selected_channels": (
                self.channel_info.get("selected_channels", [])
                if self.channel_info
                else []
            ),
            "n_selected_channels": (
                len(self.channel_info.get("selected_channels", []))
                if self.channel_info
                else 0
            ),
            "USE_REFERENCE_CHANNELS": (
                self.channel_info.get("USE_REFERENCE_CHANNELS", False)
                if self.channel_info
                else False
            ),
...
def predict_collate_fn(batch):
    ...
    batch_channel_mask = (
        torch.stack(channel_mask_list, dim=0) if channel_mask_list else None
    )
...
class PredictionDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        if not (
            self.file_path.endswith(".fif")
            or self.file_path.endswith(".ds")
            or self.file_path.endswith(".meg")
        ):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "predict" or stage is None:
            ...
            if len(self.predict_dataset) > 0:
                ...
                logger.info(
                    f"Prediction dataset setup: {len(self.predict_dataset)} samples"
                )
    ...
    def predict_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        predict_config = self.dataloader_config.get(
            "predict", self.dataloader_config.get("test", {})
        )
        # Check that shuffle is False for prediction
        if predict_config.get("shuffle", True):
            logger.warning(
                "Shuffle should be False for prediction dataloader, setting to False"
            )
