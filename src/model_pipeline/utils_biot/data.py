...
def padded_collate_fn(batch):
    ...
    If each item is (data, label) returns (batch_data, batch_label, window_mask).
    If each item is (data, label, meta) returns (batch_data, batch_label, window_mask, channel_mask, meta_list).

    Note: Channel masks are extracted from metadata and stacked into a batch tensor for proper per-sample masking.
    """
    ...
    for i, (data, label) in enumerate(zip(data_list, label_list)):
        ...
        # Extract channel mask for this sample
        if has_meta and meta_list:
            ...
            if ch_mask is not None:
                ...
                # Default: all channels valid (use number of channels from data)
                n_channels = data.shape[1]
                channel_mask_list.append(torch.ones(n_channels, dtype=torch.bool))
        else:
            # No metadata, assume all channels valid
            n_channels = data.shape[1]
            channel_mask_list.append(torch.ones(n_channels, dtype=torch.bool))

    batch_data = torch.stack(padded_data, dim=0)
    batch_label = torch.stack(padded_labels, dim=0)
    batch_window_mask = torch.stack(window_mask_list, dim=0)  # (B, N) - 1=real, 0=padded

    # Stack channel masks: (B, C) - 1=valid, 0=masked
    ...
    if has_meta and meta_list is not None:
        # Return with batch-level channel mask separated from metadata
...
class PredictionDataModule(L.LightningDataModule):
    ...
    def predict_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # Custom collate function for prediction
