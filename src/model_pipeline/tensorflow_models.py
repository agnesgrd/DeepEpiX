...
from model_pipeline.utils import (
    load_obj,
    compute_window_ppa,
    compute_window_upslope,
    compute_window_std,
    compute_window_average_slope,
    compute_window_downslope,
    compute_window_sharpness,
    compute_gfp,
    find_peak_gfp,
)
...
def test_model_dash(
    model_name, X_test_ids, output_path, threshold=0.5, adjust_onset=True, subject=None
):
    ...
    if adjust_onset == "Yes":
        ...
        for win in range(0, X_test_ids.shape[0]):
            ...
            if y_pred_probas[win] > threshold:
                ...
                times = np.linspace(
                    0, window.shape[0] / params.sfreq, window.shape[0]
                )  # Time vector

                peak_time = find_peak_gfp(gfp, times)  # Find max GFP time
                adjusted_onset = (
                    (y_timing_data[win] - window.shape[0] / 2) / params.sfreq
                ) + peak_time  # Align event to GFP peak
